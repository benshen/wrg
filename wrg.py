...
class New:
    form = web.form.Form(
        web.form.Textarea(
            'job', web.form.notnull,  rows=8, cols=80, description='JOB'),
        web.form.Textarea(
            'risk', web.form.notnull, value='None', rows=8, cols=80, description='RISK'),
        web.form.Textarea(
            'plan', web.form.notnull, rows=8, cols=80, description='PLAN'),
        # web.form.Button('Post', type = 'submit'), # don't forget to add 'type
        # = submit'
        # don't forget to add 'type = submit'
        web.form.Button(
            'Post', type='submit', class_='btn btn-success btn-sm'),
    )
...
class Edit:
    form = web.form.Form(
        web.form.Textarea(
            'job', web.form.notnull,  rows=8, cols=80, description='JOB'),
        web.form.Textarea(
            'risk', web.form.notnull, rows=8, cols=80, description='RISK'),
        web.form.Textarea(
            'plan', web.form.notnull, rows=8, cols=80, description='PLAN'),
        # don't forget to add 'type = submit'
        web.form.Button(
            'Edit', type='submit', class_='btn btn-success btn-sm'),
    )
    ...
    def POST(self, reportid):
        ...
        if not form.validates():
            ...
            if report == None:
                raise web.seeother('/')
            return render.edit(report, form)
        model.update_report_by_id(
            int(reportid), form.d.job, form.d.risk, form.d.plan)
        raise web.seeother('/')


class Download:
    form = web.form.Form(
        web.form.Textarea('attend_staff', web.form.notnull,
                          value='Yan Junping/Shen Junyong/Li Junjie/Chen Xuemei/Fan Yufeng/Zhou Xiaoyu/Zhang Jianhua/Yu Xuebiao/Gao Guiliang/Chen Jiaba/Lan Yuanheng/Lin Baoguang', rows=5, cols=80, description='Attend Staff'),
        web.form.Textbox(
            'absent_staff', value='', size=77, description='Absent Staff'),
        web.form.Textbox('meeting_direct', web.form.notnull,
                         value='Yan Junping', size=20, description='Meeting Direct'),
        web.form.Textbox('record_staff', web.form.notnull,
                         value='Shen Junyong', size=20, description='Recordd Staff'),
        # don't forget to add 'type = submit'
        web.form.Button(
            'Download', type='submit', class_='btn btn-success btn-sm'),
    )
